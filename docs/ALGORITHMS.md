...
1. [Urgency関数](#1-urgency関数)
2. [確率的LCトリガー](#2-確率的lcトリガー)
3. [ST-Boundary制約](#3-st-boundary制約)
4. [安全距離計算](#4-安全距離計算)
5. [QP定式化](#5-qp定式化)
6. [適応的安全マージン](#6-適応的安全マージン)
7. [TTC計算](#7-ttc計算)
8. [LCスケジューリング全体像](#8-lcスケジューリング全体像)
...
**Level 2戦略制御（0.1s周期）**: QP最適化（Section 5）の**主要制約**として使用。
...
**Level 2戦略制御（0.1s周期）**: 各車両に対して毎周期実行。
**入力**: 
  - 現在状態 $(s_0, v_0, a_0)$
  - 前後車両の予測軌道（ST-Boundary構築用）
  - Urgency（適応的マージン調整用）
**出力**: 最適加速度列 $[a_0, a_1, ..., a_{N-1}]$ → 先頭 $a_0$ を車両に適用
